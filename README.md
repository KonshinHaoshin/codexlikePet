...
开启“允许宠物互相聊天”后，屏幕上同时显示的不同宠物会偶尔进行一轮很短的对话。
这不是用户聊天的替代品：每只宠物只负责自己的角色和气泡，另一只可以保持安静，
并且会避开聊天窗口、暂停/安静模式和正在生成回复的情况。如果两只宠物位于同一块
显示器且都允许散步，应用会先安排它们走到附近，再显示交头接耳的气泡；不同显示器
之间不会强行移动宠物。
